...
flat_ND = 'flat_13144_flat_ND.csv' #nom du fichier 
flat_empty = 'flat_13144_flat_empty.csv'


file= 'CCD2'  #nom du dossier 
filelist = os.listdir("/home/julie/Téléchargements/CCD2-20240307T134109Z-002/"+str(file)) #liste des fichiers sur mon ordi
...
df = merge_data(filelist, ['flat_13144_flat_ND.csv','flat_13144_flat_empty.csv'])
...
def process(flat0, flat1):
    ...
    r.append((file_name_flat0,file_name_flat1,99,mean,std))
    for p in range (16):
        ...
        r.append((file_name_flat0,file_name_flat1,p, mean, std))
 
    res = pd.DataFrame(r ,columns=['flat0','flat1','ampli','mean','var'])
    
    return res


def fit_lin (diff_var,mean):
    mean = mean.to_numpy()
...
def fit (tab): 
    ...
    """
    c = []
    l=np.linspace(0, 300000,100000)
    for p in range(16):
        idx = tab[tab['ampli']==p].index
        mean = tab['mean'][idx] 
        var = tab['var'][idx]
        a,b = fit_lin(var, mean)
        y,y_params,v = fit_quadra(var, mean)
        turnoff = np.max(y(l))
        c.append((p,y_params[0],y_params[1],y_params[2],turnoff))
        
    parameters = pd.DataFrame(c ,columns=['ampli','a','b','c','turnoff'])
...
for i in range (len(df)):
    flat0 = path+df['full_path_flat0'][i]
    flat1 = path+df['full_path_flat1'][i]
    res  = process(flat0,flat1)
    resa = pd.concat([res,resa])
 
param_fit=fit(resa)


def plot_ampli (data, parameters):
    """
    ...
    l=np.linspace(0, 50000,50)
    fig, axs = plt.subplots(4,4,sharex=True,figsize=(18,9))
    
    for i in range (16):
        irow = int(i/4)
        icol = i%4
        idx = data[data['ampli']==i].index
        mean = data['mean'][idx] 
        var = data['var'][idx]
        idx = parameters[parameters['ampli']==i].index
        ...
        axs[irow,icol].text(6000/2,100, "HDU {} \nGain = {}\nTurnoff = {}  ".format(i+1,np.round(1/b,2),np.round(turnoff,4)), fontsize=12)
