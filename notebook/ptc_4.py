...
parser.add_option('--imList', type=str,
                  default='flat_13144_flat_ND.csv',
                  help='List of images to process[%default]')

parser.add_option('--image', type=str,
                  default='flat_13144_flat_empty.csv',
                  help='List of output images [%default]')

opts, args = parser.parse_args()

flat_ND = opts.imList
flat_empty = opts.image

flat = [flat_ND,flat_empty]
...
def fit_quadra (diff_var,mean) :
    ...
    y_params = np.polyfit(x_data, y_data, 2)  #équation de degré 2   recup cov : , full=False, cov=True
        ...
        for s in sensor:
            for p in range(1,17):
                ...
                if not list(idx):
                    ...
                    y,y_params = fit_quadra(var, mean)
                    max_var = np.max(var)
...
def plot_ampli (data, parameters):
    ...
    for r in raft:
        for s in sensor:
            idx = data[(data['raft']==r)&(data['sensor']==s)].index
            if not list(idx):
                continue
            else:
                ...
                for i in range (16):
                    irow = int(i/4)
                    icol = i%4
                    idx = data[(data['ampli']==i+1)&(data['raft']==r)&
                               (data['sensor']==s)].index
                    mean = data['mean'][idx] 
                    var = data['var'][idx]
            ...
            fig.suptitle('PTC for '+r+s+ 'run13144 : '+str(len(raft)-1)+
                         ' illuminations (unbias '+str(UnBias)+')')#mofifier nom de figure pour automatiser
